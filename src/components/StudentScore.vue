<template>
	<div class="student-score">
		<div class="score-btns">
			<el-button @click="downloadDate">按学生统计</el-button>
			<el-button @click="downloadDateMinus">按时间统计</el-button>
		</div>
		...
		<div class="search-box">
			<el-form ref="form1" :model="form1" label-width="80px">
				...
				<el-form-item label="学号:">
					<el-input size="mini" v-model="form1.stu_id" placeholder="请输入学号"></el-input>
				</el-form-item>

				<el-form-item label="班级">
					<el-select v-model="form1.class" size="mini" placeholder="请选择班级">
				...
				<el-button style="margin-left: 20px;" type="primary" class="el-icon-search">搜索</el-button>
			</el-form>
		</div>
	...
	export default {
		...
		methods: {
			downloadDate: function() {
				var url = this.api.scoreUrl

				console.log(url)

				this.axios.get(url).then(res => {
					this.score_list = res.data.list
					this.getShowList();
				})
			},

			downloadDateMinus: function() {
				var url = this.api.scoreMinusUrl
			...
			dealShowDialog: function(index) {
				...
				var url = this.api.studentScore;
				this.axios.get(url, {
					params: score_item
				}).then(res => {
					this.socre_item = res.data.list
					console.log(this.socre_item)
				})
			},
			//处理扣分
